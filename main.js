...
window.addEventListener("load", function () {
  ...
  var nodejsBar = this.document.querySelector(".nodejs-skill");
  var expressjsBar = this.document.querySelector(".express-skill");
  var bootstrapBar = this.document.querySelector(".bootstrap-skill");
  var projectsCard = this.document.querySelector(".projects");
  var experiencesCard = this.document.querySelector(".experiences");
  var certificatesSubtitle = this.document.querySelector(
    ".certificates .subtitulo"
  );
  var firstColumnCertificates = this.document.querySelectorAll(
    ".certificate_first_column"
  );
  var secondColumnCertificates = this.document.querySelectorAll(
    ".certificate_second_column"
  );
  ...
  window.addEventListener("scroll", function () {
    ...
    if (height < 700) {
      ...
      skillsImages.forEach((image) => {
        ...
        skillUp(nodejsBar, 30);
        skillUp(djangoBar, 79);
        skillUp(javascriptBar, 72);
        skillUp(htmlBar, 90);
        skillUp(cssBar, 87);
        skillUp(mysqlBar, 50);
        skillUp(expressjsBar, 40);
        skillUp(bootstrapBar, 50);
      });
      ...
      if (scroll >= 460 && scroll <= 1086) {
        ...
        skillsImages.forEach((image) => {
          ...
          skillDown(cssBar);
          skillDown(mysqlBar);
          skillDown(expressjsBar);
          skillDown(bootstrapBar);
        });
      } else if (scroll > 1086) {
        skillsCard.style.transform = "translateX(-2000px)";
        skillsImages.forEach((image) => {
